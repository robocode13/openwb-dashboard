...
import { backwardsReadingsIterator } from './iterator';
...
vi.mock('./readingsRepository', async () => {
	return {
		readDailyFile: vi.fn().mockImplementation(async (date) => {
			if (date.getDate() === 1) {
				return [
					{ dateTime: new Date('2024-08-01T00:00:00'), batterySoc: 74 },
...
describe('iterator', () => {
	it('should return first reading', async () => {
		const reader = backwardsReadingsIterator(new Date(2024, 7, 2), new Date(2024, 7, 1), config);

		const reading = (await reader.next()).value;

		expect(reading.dateTime).toStrictEqual(new Date('2024-08-02T00:00:00'));
		expect(reading.batterySoc).toBe(34);
	});
	...
	it('should return first three readings', async () => {
		...
		expect(reading2.dateTime).toStrictEqual(new Date('2024-08-01T23:55:00'));
		expect(reading2.batterySoc).toBe(35);
	...
	it('should return done when no next reading available', async () => {
		const reader = backwardsReadingsIterator(new Date(2024, 7, 1), new Date(2024, 7, 1), config);
