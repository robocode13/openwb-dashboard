<script lang="ts">
	...
	import { formatPower, formatValue } from '$lib/format';
...
<div class="m-1 d-flex justify-content-center">
	...
	<svg width="440" height="500" stroke="currentColor">
		<g transform="translate(160 0)" class="yellow">
			<SvgRing text="{formatValue(energy.pv)} kWh" subText={power ? formatPower(power.pv) : ''}>
				<path
		...
		<g transform="translate(0 160)" class="gray">
			<SvgRing text="{energy.gridIn > energy.gridOut ? '+' : ''}{formatValue(energy.gridIn - energy.gridOut)} kWh">
				<path
		...
		<g transform="translate(160 320)" class="red">
			<SvgRing text="{formatValue(energy.home)} kWh" subText={power ? formatPower(power.home) : ''}>
				<path
		...
		<g transform="translate(120 410) scale(0.75)" class="dark-red">
			<SvgRing text="{formatValue(energy.house)} kWh" subText={power ? formatPower(power.house) : ''}>
		...
		<g transform="translate(230 410) scale(0.75)" class="dark-red">
			<SvgRing text="{formatValue(energy.wallbox)} kWh" subText={power ? formatPower(power.wallbox) : ''}>
				<svg width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
					<path
		...
		<g class="yellow" fill="currentColor" stroke-width="0" font-size="12">
			...
			<text x="125" y="235">{formatValue(energy.gridOut)} kWh</text>
			<text x="315" y="216" text-anchor="end">{power ? formatPower(power.batteryIn) : ''}</text>
			<text x="315" y="235" text-anchor="end">{formatValue(energy.batteryIn)} kWh</text>
			<text x="215" y="315" text-anchor="end">
				{power ? formatPower(power.directPv) : ''}
			</text>
			<text x="225" y="315">
				{formatValue(energy.directPvConsumption)} kWh
			</text>
		</g>
		...
		<g class="gray" fill="currentColor" stroke-width="0" font-size="12">
			<text x="155" y="395" text-anchor="end">{formatValue(energy.gridIn)} kWh</text>
		</g>
		...
		<g class="green" fill="currentColor" stroke-width="0" font-size="12">
			<text x="285" y="395">{formatValue(energy.batteryOut)} kWh</text>
		</g>
	</svg>
